...
@invocation("tracery", title="Tracery", tags=["string", "random", "wildcards"], category="string", version="1.0.0")
class TraceryInvocation(BaseInvocation):
    """Applies a Tracery grammar to a given string."""

    seed: int = InputField(default=0, title="Seed", description="The seed for the randomization.")
    grammar_str: str = InputField(default="""{ "test": [ "one", "two", "three" ] }""", title="Grammar JSON", description="Grammar", ui_component=UIComponent.Textarea)
    main_input: str = InputField(default="", title="Input String", description="Input", ui_component=UIComponent.Textarea)

    def invoke(self, context: InvocationContext) -> StringOutput:
        grammar = Grammar(json.loads(self.grammar_str))
        random.seed(self.seed)
        final: str = grammar.flatten(self.main_input)
        return StringOutput(value=((final or "")))
    


@invocation("tracery_template_merger", title="Tracery Template Merger", tags=["string" ], category="string", version="1.0.0", use_cache=False,)
class TraceryTemplateMergerInvocation(BaseInvocation):
    """Loads a txt file from the system in to a JSON dictionary."""

    #template_a: str = InputField(default="", title="Template A", description="")
    #template_b: str = InputField(default="", title="Template B", description="")
    ...
    def invoke(self, context: InvocationContext) -> StringOutput:
        ...
        return StringOutput(value=(json.dumps(dict)))
...
class NodeAction(object):  # has a 'raw' attribute
    ...
    def activate(self):
        ...
        if self.type == 0:
            ...
            grammar.flatten(self.target, random.randint(0, 9999999999999), True)
